...
async def test_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await telegram_client.send_message_to_group(
        ...
        "Testing systems, everything functional, sir."
    )
...
async def init_game(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_chat.type == "private":
        await telegram_client.send_message_to_group(group_id, "Please use this command in a group, sir.")
        return

    if game_manager.get_game(group_id):
        await telegram_client.send_message_to_group(group_id, "A game already exists, sir.")
        return
...
async def join_game(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    game = game_manager.get_game(group_id)
    if not game:
        await telegram_client.send_message_to_group(group_id, "That game no longer exists, sir.")
        return

    if game.locked:
        await telegram_client.send_message_to_group(group_id, "The game has already begun, sir!")
        return
    ...
    if added:
        await telegram_client.send_message_to_group(group_id, f"{user.username} joined the game.")
    else:
        await telegram_client.send_message_to_group(group_id, "You’re already in this game, sir.")
...
async def handle_dm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not reply_to:
        ...
        await update.message.reply_text("Please reply to the prompt message, sir.")
        return
    ...
    for game in game_manager.active_games.values():
        if user_id in game.players:
            ...
            if prompt_idx is None:
                await update.message.reply_text("That message isn't a valid prompt, sir.")
                return

            # store the answer
            game.pending_answers[user_id][prompt_idx] = text
            await update.message.reply_text("✅ Answer recorded, sir.")
            break
...
async def end_game(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    game = game_manager.get_game(group_id)
    if not game:
        await telegram_client.send_message_to_group(group_id, "No active game to end, sir.")
        return
